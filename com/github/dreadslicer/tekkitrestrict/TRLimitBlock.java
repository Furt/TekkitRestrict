...
public class TRLimitBlock {
	...
	/*
	 * private List<String> getPermLimits(Player p){ LinkedList<String> types =
	 * new LinkedList<String>();
	 * 
	 * //loop through the player's permissions. String[] perms =
	 * TRPermHandler.getPermissions(p,"tekkitrestrict.limit."); for(int
	 * i=0;i<perms.length;i++){ //tekkitrestrict.limit.[l-k].[data].[limit]
	 * //tekkitrestrict.limit.[l-k]..[limit]
	 * if(perms[i].startsWith("tekkitrestrict.limit.")){ String perm =
	 * perms[i].replace("tekkitrestrict.limit.", ""); if(perm.contains(".")){
	 * perm = perm.replace(".","!"); String[] tperm = perm.split("!");
	 * if(tperm.length == 2){ //single or multiple values without a datavalue //
	 * with a limit. //int v = Integer.valueOf(tperm[0]); ItemStack[] tr =
	 * TRNoItem.getRangedItemValues(tperm[0]); int l =
	 * Integer.valueOf(tperm[1]); for(int kk = 0;kk< tr.length;kk++){
	 * types.add(tr[kk].id+" "+l); }
	 * 
	 * //tekkitrestrict.log.info(v+" "+l); } else if(tperm.length == 3){
	 * //single value with datavalue and limit int v =
	 * Integer.valueOf(tperm[0]); int d = Integer.valueOf(tperm[1]); int l =
	 * Integer.valueOf(tperm[2]); types.add(v+":"+d+" "+l);
	 * //tekkitrestrict.log.info(v+":"+d+" "+l); } } } }
	 * 
	 * return types; }
	 */
	// static members.
	...
	public static void reload() {
		...
		for (String ir : ccl) {
			try {
				String[] g = ir.split(" ");
				TRCacheItem.processItemString("limiter", "", g[0],
						Integer.valueOf(g[1]));
				/*
